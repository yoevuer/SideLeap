package hunoia.luno.system.shizuku
...
import hunoia.luno.system.shizuku.ShizukuCommandService
...
object ShizukuCommand {
    ...
    fun enablePackageForLauncher(context: Context, packageName: String): EnablePackageResult {
        if (!ShizukuRuntime.isAvailable()) {
            if (BuildConfig.DEBUG) Log.d("LunoLauncher", "shizuku_enable_launcher: unavailable")
            return EnablePackageResult(false, packageName, error = "shizuku unavailable")
        }
        if (ShizukuRuntime.isPreV11OrUnsupported()) {
            if (BuildConfig.DEBUG) Log.d("LunoLauncher", "shizuku_enable_launcher: unsupported")
            return EnablePackageResult(false, packageName, error = "shizuku unsupported")
        }
        if (!ShizukuRuntime.checkPermission()) {
            if (BuildConfig.DEBUG) Log.d("LunoLauncher", "shizuku_enable_launcher: permission denied")
            return EnablePackageResult(false, packageName, error = "permission denied")
        }

        if (BuildConfig.DEBUG) Log.d("LunoLauncher", "shizuku_enable_launcher: target=$packageName")

        var result = enableWithCachedService(context, packageName)
        if (result != null) {
            if (BuildConfig.DEBUG) Log.d("LunoLauncher", "shizuku_enable_launcher: result=${result.success}")
            return result
        }
        ...
        if (result != null) {
            if (BuildConfig.DEBUG) Log.d("LunoLauncher", "shizuku_enable_launcher: result=${result.success} retry")
            return result
        }

        if (BuildConfig.DEBUG) Log.d("LunoLauncher", "shizuku_enable_launcher: failed after retry")
