...
object AppSearch {
    ...
    private fun prefixRank(context: Context, app: AppInfo, tokens: List<String>): Int {
    ...
    private fun containsRank(context: Context, app: AppInfo, tokens: List<String>): Int {
        if (tokens.isEmpty()) return 0
        val index = buildIndex(context, app.label)
        return if (matchesTokens(index.initials, tokens) || matchesTokens(index.pinyin, tokens) || matchesTokens(index.raw, tokens)) 1 else 0
    }

    private fun buildIndex(context: Context, text: String): AppSearchIndex {
