...
- 长滑动即时触发模式的动作选择不显示硬上限，内部保留 50 个动作软保护；短滑、单击、长按和非即时长滑仍保持单动作语义。
- `GestureButton.longSlideActionPanelStyles` 按长滑方向保存动作面板样式，默认仍为弧形；旧列表样式按弧形处理。
- 触钮设置页的每个长滑方向可单独选择弧形或网格。
- 运行时长滑多动作面板按触发方向读取样式；弧形支持多层并限制在屏幕内，网格按可用屏幕空间自适应列数和行数。
...
### 虚拟鼠标光标（`feat/virtual-mouse-optimization`）

- 新增「虚拟鼠标」动作，放入动作选择器「工具」分类。
- 滑动直接触发时在现有手势事件流内接管后续 MOVE/UP，同一手指相对移动动态色圆环准星，抬起后隐藏光标并延迟执行无障碍点击。
- 面板选择、单击、长按等已断触路径通过 `VirtualMouseOverlay` 捕获下一次触摸，使用同一光标绘制组件，取消事件只退出不点击。
- 鼠标模式通过 `SideGestureRuntimeState.isMouseMode` 隐藏所有手势触钮，点击完成后再恢复，避免模拟点击打到触发区。
- 手势设置页「虚拟鼠标」分组可调整 X/Y 灵敏度、加速曲线、连续模式、光标大小/透明度、点击动画、拖尾样式和停留长按。
- 光标初始位置为屏幕中心偏上；连续模式下复用上次光标位置；移动计算支持防抖、慢速精准和加速曲线。
- 光标使用细圆环准星、微小中心点、黑白双描边和动态主题色；拖尾支持关闭、残影点和光带三种样式，默认残影点。
- 停留长按可选开启，停住超过配置延迟后执行无障碍长按，抬手不再触发普通点击。
- 手指离开边缘后再滑回屏幕边缘可取消鼠标模式。
- 虚拟鼠标设置入口改为手势设置页中的 BottomSheet，主页面只显示摘要，避免上下滑动页面时误触大量滑杆；连续模式支持空闲超时自动退出。
