...
- `GestureButton.longSlideActionPanelStyles` 按长滑方向保存动作面板样式，默认仍为弧形。
- 触钮设置页的每个长滑方向可单独选择弧形、自适应列表或自适应网格。
- 运行时长滑多动作面板按触发方向读取样式；弧形会按动作数量调整展开角度，网格按 1-4/5-6/7-10 个动作自适应 1/2/3 列。
