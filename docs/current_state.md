...
### 虚拟鼠标光标（`feat/virtual-mouse-cursor`）

- 新增「虚拟鼠标」动作，放入动作选择器「工具」分类。
- 滑动直接触发时在现有手势事件流内接管后续 MOVE/UP，同一手指相对移动蓝色圆点光标，抬起后隐藏光标并延迟执行无障碍点击。
...
- 手势设置页新增「虚拟鼠标」全局分组，可调整 X/Y 灵敏度、加速曲线、连续模式、光标大小/颜色/透明度、外圈、阴影、点击动画和拖尾轨迹。
- 光标初始位置改为屏幕中心偏上；移动计算支持加速曲线；手指离开边缘后再滑回屏幕边缘可取消鼠标模式。
